...
class SecurityScanner(BaseScanner):
    ...
    def _scan_file(
        ...
        # SEC-003: eval()/exec() usage — Python AST
        tree = python_asts.get(fi.path)
        if tree is not None:
            for func_name in ("eval", "exec"):
                for lineno in get_function_calls(tree, func_name):
                    findings.append(Finding(
                        rule_id="SEC-003",
                        category="security",
                        severity="fail",
        ...
        # SEC-003 fallback for JS/TS: regex-based eval detection
        ...
        # SEC-006: dangerouslySetInnerHTML without DOMPurify
        if fi.language in ("javascript", "typescript"):
            has_dompurify = bool(_DOMPURIFY.search(content))
            for i, line in enumerate(lines, 1):
        ...
        # SEC-007: Hardcoded connection strings
        for i, line in enumerate(lines, 1):
        ...
        # SEC-010: Path traversal via user input
        for i, line in enumerate(lines, 1):
